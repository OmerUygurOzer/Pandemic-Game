...
public:
	...
	void fillTestDecks() // fills the discard and draw decks for testing
	{
		...
		for(int i = 0; i<30; i++)  //fill draw deck with first 30 cards
		{
		...
		for(int i = 30; i<48; i++)  // fill discard deck with final 18 cards
		{
			infectCard.cardType = "City";
			infectCard.cardDescription = cityname[i];
			infectCard.city = i;
			infectCard.color = 0;  // color not important
			discardInfectionDeck.push_back(infectCard);//add onto infectiondeck after info populated 
		}
	...
	void testTopCard(void) // test to make sure top card is correct
	{
		...
		TS_ASSERT_EQUALS(testCard1.city, testCard2.city);
	}
